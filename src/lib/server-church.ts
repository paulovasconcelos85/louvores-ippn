...
export async function findUsuarioAcessoByAuthOrEmail(
  supabaseAdmin: ReturnType<typeof getSupabaseAdmin>,
  authUserId: string,
  email?: string | null
) {
  const byAuth = await supabaseAdmin
    ...
    .limit(1)
    .maybeSingle<UsuarioAcessoBasico>();
...
async function getAuthorizedChurchIdsForAccess(
  supabaseAdmin: ReturnType<typeof getSupabaseAdmin>,
...
async function getPrimaryChurchIdForAccess(
  supabaseAdmin: ReturnType<typeof getSupabaseAdmin>,
  acesso: UsuarioAcessoBasico | null
) {
  if (!acesso?.id) {
...
export async function resolveCurrentIgrejaId(preferredIgrejaId?: string | null, request?: Request | null) {
  ...
  return (
    await getPrimaryChurchIdForAccess(supabaseAdmin, acesso) ||
...
export async function resolveAuthorizedCurrentIgrejaId(
  ...
  const igrejaIds = new Set(await getAuthorizedChurchIdsForAccess(supabaseAdmin, acesso));
