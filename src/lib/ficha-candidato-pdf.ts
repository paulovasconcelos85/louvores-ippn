...
export function gerarFichaCandidatoPdf(pessoa: FichaCandidatoPessoa, opts: GerarFichaCandidatoOpts) {
  ...
  doc.setFontSize(15);
  doc.text(tr('FICHA DE CANDIDATO À MEMBRESIA', 'FICHA DE CANDIDATO A MEMBRESÍA', 'MEMBERSHIP CANDIDATE FORM'), marginX, headerH / 2 + 2.5);
  ...
  const campo = (x: number, w: number, h: number, label: string, valor: string) => {
    ...
    doc.text(labelLinhas, x + 1.5, y + 2.6);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8.5);
    doc.setTextColor(...COR_TEXTO);
    const valorY = y + 2.6 + labelLinhas.length * 3 + 1.5;
    const linhas = doc.splitTextToSize(valor || '', w - 3) as string[];
    doc.text(linhas.slice(0, Math.max(1, Math.floor((y + h - valorY) / 3.4) + 1)), x + 1.5, valorY);
  };

  const linhaCampos = (celulas: { w: number; label: string; valor: string }[], h = 7.5) => {
    let x = marginX;
  ...
  const desenharCheckbox = (x: number, yCentro: number, marcado: boolean) => {
    ...
    if (marcado) {
      ...
      doc.line(x + 0.4, boxY + 1.6, x + 1.2, boxY + boxSize - 0.3);
      doc.line(x + 1.2, boxY + boxSize - 0.3, x + boxSize - 0.3, boxY + 0.3);
  ...
  type Segmento =
    ...
    | { tipo: 'linha_texto'; label: string; valor: string; largura?: number };

  const linhaMista = (segmentos: Segmento[], h = 6) => {
    doc.setDrawColor(...COR_BORDA);
    ...
    for (const seg of segmentos) {
      if (seg.tipo === 'checkbox') {
        ...
        doc.setTextColor(...COR_TEXTO);
        doc.text(seg.label, textX, yCentro + 1.2);
        x = textX + doc.getTextWidth(seg.label) + 4;
      } else {
        doc.setTextColor(...COR_TEXTO);
        doc.text(seg.label, x, yCentro + 1.2);
        const labelW = doc.getTextWidth(seg.label) + 2;
        const lineEndX = seg.largura ? x + seg.largura : marginX + contentW - 2;
        doc.setDrawColor(...COR_BORDA);
        doc.line(x + labelW, yCentro + 1.6, lineEndX, yCentro + 1.6);
        if (seg.valor) {
          doc.setFont('helvetica', 'normal');
          doc.setFontSize(7.3);
          const valorLinhas = doc.splitTextToSize(seg.valor, lineEndX - (x + labelW) - 1) as string[];
          doc.text(valorLinhas[0] || '', x + labelW + 1, yCentro + 0.6);
        }
        x = lineEndX + 3;
      }
    }
  ...
  doc.setFontSize(7.3);
  doc.setDrawColor(...COR_BORDA);
  doc.line(marginX + 105, y - 2, marginX + contentW - 2, y - 2);
  if (pessoa.transferido_ipb && pessoa.transferencia_ipb_origem) {
    doc.text(pessoa.transferencia_ipb_origem, marginX + 106, y - 2.6);
  }
  linhaMista([
    { tipo: 'linha_texto', label: tr('Data do Batismo infantil', 'Fecha del Bautismo infantil', 'Infant Baptism date'), valor: formatarDataPt(pessoa.data_batismo), largura: 18 },
    { tipo: 'linha_texto', label: tr('Data da Profissão de Fé', 'Fecha de Profesión de Fe', 'Profession of Faith date'), valor: formatarDataPt(pessoa.data_profissao_fe), largura: 18 },
    {
      tipo: 'linha_texto',
  ...
  doc.line(marginX + 90, y - 2, marginX + contentW - 2, y - 2);
  if (pessoa.transferido_outra_denominacao) {
    doc.text(pessoa.transferido_outra_denominacao, marginX + 91, y - 2.6);
  }
  linhaMista([
    { tipo: 'linha_texto', label: tr('Data do Batismo', 'Fecha del Bautismo', 'Baptism date'), valor: !pessoa.transferido_ipb ? formatarDataPt(pessoa.data_batismo) : '', largura: 18 },
  ...
  doc.line(marginX + 110, y - 2, marginX + contentW - 2, y - 2);
  if (pessoa.transferencia_jurisdicao_sem_carta) {
    doc.text(pessoa.transferencia_jurisdicao_sem_carta, marginX + 111, y - 2.6);
  }
  linhaMista([
    { tipo: 'linha_texto', label: tr('Data do Batismo', 'Fecha del Bautismo', 'Baptism date'), valor: '', largura: 18 },
  ...
  const assinanteNome = opts.assinanteNome?.trim() || '';

  const rowAprovadoH = 9;
  doc.setDrawColor(...COR_BORDA);
  doc.rect(marginX, y, contentW, rowAprovadoH);
  const col1W = 22;
  const col2W = contentW * 0.5;
  desenharCheckbox(marginX + 2, y + rowAprovadoH / 2, assinado);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7.3);
  doc.setTextColor(...COR_TEXTO);
  doc.text(tr('Aprovado', 'Aprobado', 'Approved'), marginX + 6, y + rowAprovadoH / 2 + 1.2);
  doc.line(marginX + col1W, y, marginX + col1W, y + rowAprovadoH);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(6.3);
  doc.setTextColor(...COR_LABEL);
  doc.text(tr('Nome completo do pastor/presbítero', 'Nombre completo del pastor/presbítero', "Pastor/elder's full name"), marginX + col1W + 1.5, y + 2.6);
  if (assinanteNome) {
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.setTextColor(...COR_TEXTO);
    doc.text(assinanteNome, marginX + col1W + 1.5, y + 6.8);
  }
  doc.line(marginX + col1W + col2W, y, marginX + col1W + col2W, y + rowAprovadoH);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(6.3);
  doc.setTextColor(...COR_LABEL);
  doc.text(tr('Assinatura:', 'Firma:', 'Signature:'), marginX + col1W + col2W + 1.5, y + 2.6);
  if (pessoa.assinatura_ficha) {
    try {
      doc.addImage(pessoa.assinatura_ficha, 'PNG', marginX + col1W + col2W + 1.5, y + 3, contentW - col1W - col2W - 4, rowAprovadoH - 4);
    } catch {
  ...
  const rowAguardarH = 8;
  doc.setDrawColor(...COR_BORDA);
  doc.rect(marginX, y, contentW, rowAguardarH);
  desenharCheckbox(marginX + 2, y + rowAguardarH / 2, false);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7.3);
  doc.setTextColor(...COR_TEXTO);
  doc.text(tr('Aguardar', 'Esperar', 'Wait'), marginX + 6, y + rowAguardarH / 2 + 1.2);
  doc.line(marginX + col1W, y, marginX + col1W, y + rowAguardarH);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(6.3);
  doc.setTextColor(...COR_LABEL);
  doc.text(tr('Motivo:', 'Motivo:', 'Reason:'), marginX + col1W + 1.5, y + 2.6);
  y += rowAguardarH;
